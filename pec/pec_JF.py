...
with open("time_JF_{}_k={}.csv".format(mole_name, k_svd), 'w') as file:
...
for d in dist:
    ...
    def cost(Jvec_tmp):
        # Jについてのデータを受け取る]
        Jvec_tmp = Jvec_tmp.reshape((k_jf, -1))
        J_tmp = np.array(
        ...
        for x in range(k_jf):
            for p, q, r, s, i, j in itertools.product(range(half_nqubit),
                                                      repeat=6):
                V_jf_tmp[p][q][r][s] += lam[x] * U[x][p][i] * (np.conjugate(
                    U[x])).T[i][q] * J_tmp[x][i][j] * U[x][r][j] * (
                        np.conjugate(U[x])).T[j][s]
    ...
    Jvec = np.array([symmetric_matrix_to_vector(J[i]) for i in range(k_jf)])
    ...
    # 最適化の結果からJを作成
    Jvec = Jvec.reshape((k_jf, -1))
    J = np.array([vector_to_symmetric_matrix(Jvec[i]) for i in range(k_jf)])

    # 最適化した結果を保存
    with open("J_data/J_{}_k={}_d={}.csv".format(mole_name, k_svd, d),
              'w') as file:
        writer = csv.writer(file, lineterminator='\n')
        writer.writerow(["J"])
    ...
    plt.legend(bbox_to_anchor=(1.05, 1),
               ...
               borderaxespad=0,
               fontsize=18)
    plt.savefig("potential_JF_{}_k={}.png".format(mole_name, k_svd),
                bbox_inches='tight')
    ...
    # 結果をcsvにして保存する
    with open("potential_JF_{}_k={}.csv".format(mole_name, k_svd),
              'w') as file:
        writer = csv.writer(file, lineterminator='\n')
        ...
        writer.writerow(["JF energy"])
        writer.writerows(energy_jf_hist)

    with open("time_JF_{}_k={}.csv".format(mole_name, k_svd), 'a') as file:
        writer = csv.writer(file, lineterminator='\n')
        # writer.writerow(["d", d])
